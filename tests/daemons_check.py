...
class DaemonChecks(unittest.TestCase):
    def required_services(self):
        ...
        if "ModemManager" not in ps_ef:
            Results.add_error(
                "ps -ef",
                "Modem Manager is not running. Start using: sudo start modemmanager",
            )

        if "NetworkManager" not in ps_ef:
            Results.add_error(
                "ps -ef",
                "Network Manager is not running. Start using: sudo start network-manager",
            )

    def wwan_interfaces(self):
        ifcfg = Runner.run_cmd("ifconfig -a")
        if "wwan" not in ifcfg and "wwp" not in ifcfg:
            Results.add_error(
                "ifconfig -a",
                "wwan or wwp interface is not enumerated. Please restart network-manager using: sudo stop network-manager && sudo start network-manager"
                + ifcfg,
            )
